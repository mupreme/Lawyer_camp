<header>
	...
	<script>
		function MySucessFn(){
			swal({
				title: " User...Your Registration Sucessfully Complete! Please Check Your email ",
				text: "",
				type: "success",
			...
			window.load = function(){
				window.location='http://localhost/lawyer/user_register.php';
			});
		}
		function MyCheckFn(){
			swal({
        ...
        else
        {
          ...
          //echo $sql;exit;
		   $result=mysqli_query($conn, $sql) or die(mysqli_error ($conn));
            if($result==1)
            {
                $sql2= "INSERT INTO `client` (`client_id`, `contact_number`, `full_address`, `city`, `zip_code`, `image`) VALUES ('$u_id', '$contact_number', '$full_address', '$city', '$zip_code ', '  ');";
