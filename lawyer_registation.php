<header>
	...
	<script>
		function MySucessFn(){
			swal({
				title: "Lawyer... Registration Sucessfully Complete! Please Wait for Approval",
				text: "",
				type: "success",
			...
			window.load = function(){
				window.location='http://localhost/lawyer/login.php';
			});
		}
		function MyCheckFn(){
			swal({
			...
			window.load = function(){
				window.location='http://localhost/lawyer/lawyer_register.php';
			});
		}
	</script>
</header>
<?php
	...
	if($okFlag){
        ...
        else
        {
			...
			VALUES ('$u_id','$first_Name', '$last_Name', '$email ', '$password ', 'Active', 'Lawyer')";
          //echo $sql;exit;
